...
export type SiteConfig = {
  ...
  pickup_discount:            number  // % скидки при самовывозе
  birthday_discount_percent:  number  // % скидки на день рождения
}
...
function parseValue(key: keyof SiteConfig, value: string): any {
  if (['delivery_cost', 'free_delivery_threshold', 'gift_threshold',
       'min_order_time', 'min_order_amount', 'banners_desktop', 'banners_mobile',
       'pickup_discount', 'birthday_discount_percent'].includes(key)) {
    return Number(value)
  }
